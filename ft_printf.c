...
/*   Updated: 2021/06/14 11:47:40 by ugdaniel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_conversion(t_flags *flags, va_list va, int fd)
{
	int		ret;
...
static int	ft_process_str(const char *format, va_list va, int fd)
{
	int			ret;
	t_flags		*flags;
...
int	ft_fprintf(int fd, const char *format, ...)
{
	...
	{
		va_start(va, format);
		ret = ft_process_str(format, va, fd);
		va_end(va);
	}
	return (ret);
}
...
int	ft_printf(const char *format, ...)
{
	...
	{
		va_start(va, format);
		ret = ft_process_str(format, va, STDOUT_FILENO);
		va_end(va);
	}
	return (ret);
}
